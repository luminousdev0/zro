...
public:
     ...
     *     1) we have an active Zeronode,
     *     2) we have a valid Zeronode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Zeronode address
    bool CheckSignature(const CPubKey& pubKeyZeronode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, zeronode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 1000 ZRO - checking if valid zeronode)
...
private:
    ...
    // Keep track of the used Zeronodes
    std::vector<CTxIn> vecZeronodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Zeronode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Zeronode to start the process
    ...
    /// Get Zeronode updates about the progress of mixing
...
public:
    CZeronode* pSubmittedToZeronode;
