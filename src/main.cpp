...
#if defined(NDEBUG)
# error "Zerobitcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Zerobitcoin Signed Message:\n";
...
//                                      !(pindexBIP34height->GetBlockHash() == chainparams.GetConsensus().BIP34Hash));
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (txIds.count(txHash) > 0 && (fTestNet || pindex->nHeight >= HF_ZERONODE_HEIGHT))
    ...
    // ZERONODE : MODIFIED TO CHECK ZERONODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        ...
        return state.DoS(0, error("ConnectBlock(): couldn't find zeronode or superblock payments"),
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams &chainParams) {
    ...
    zeronodeSync.UpdatedBlockTip(chainActive.Tip());
...
CAmount GetZeronodePayment(int nHeight, CAmount blockValue) {
//    CAmount ret = blockValue * 30/100 ; // start at 30%
//    int nMNPIBlock = Params().GetConsensus().nZeronodePaymentsStartBlock;
////    int nMNPIBlock = Params().GetConsensus().nZeronodePaymentsIncreaseBlock;
//    int nMNPIPeriod = Params().GetConsensus().nZeronodePaymentsIncreasePeriod;
//
//    // mainnet:
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW,
    ...
    try {
        ...
        if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
            ...
            BOOST_FOREACH(const CTransaction& tx, block.vtx) {
                ...
                BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                    ...
                    if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                        ...
                        LOCK(cs_main);
                        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                        return state.DoS(0, error("CheckBlock(ZRO): transaction %s conflicts with transaction lock %s",
            ...
            LogPrintf("CheckBlock(ZRO): spork is off, skipping transaction locking checks\n");
        }
...
bool ProcessNewBlock(CValidationState &state, const CChainParams &chainparams, CNode *pfrom, const CBlock *pblock,
    ...
    zeronodeSync.IsBlockchainSynced(true);
...
bool static AlreadyHave(const CInv &inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    ...
    */
        ...
        case MSG_ZERONODE_PAYMENT_VOTE:
            return mnpayments.mapZeronodePaymentVotes.count(inv.hash);

        case MSG_ZERONODE_PAYMENT_BLOCK:
        {
            BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
            return mi != mapBlockIndex.end() && mnpayments.mapZeronodeBlocks.find(mi->second->nHeight) != mnpayments.mapZeronodeBlocks.end();
        }

        case MSG_ZERONODE_ANNOUNCE:
            return mnodeman.mapSeenZeronodeBroadcast.count(inv.hash) && !mnodeman.IsMnbRecoveryRequested(inv.hash);

        case MSG_ZERONODE_PING:
            return mnodeman.mapSeenZeronodePing.count(inv.hash);

        case MSG_DSTX:
            return mapDarksendBroadcastTxes.count(inv.hash);

        case MSG_ZERONODE_VERIFY:
            return mnodeman.mapSeenZeronodeVerification.count(inv.hash);
    }
...
//                LogPrintf("inv.type()=%s, inv.GetCommand=%s\n", inv.type, inv.GetCommand());
                ...
                if (!pushed && inv.type == MSG_ZERONODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapZeronodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::ZERONODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_ZERONODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapZeronodeBlocks);
                    if (mi != mapBlockIndex.end() && mnpayments.mapZeronodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CZeronodePayee& payee, mnpayments.mapZeronodeBlocks[mi->second->nHeight].vecPayees) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapZeronodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::ZERONODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_ZERONODE_ANNOUNCE) {
                    if(mnodeman.mapSeenZeronodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenZeronodeBroadcast[inv.hash].second;
                ...
                if (!pushed && inv.type == MSG_ZERONODE_PING) {
                    if(mnodeman.mapSeenZeronodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenZeronodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_ZERONODE_VERIFY) {
                    if(mnodeman.mapSeenZeronodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenZeronodeVerification[inv.hash];
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv, int64_t nTimeReceived,
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        int minPeerVersion = (nHeight + 1 < HF_ZERONODE_HEIGHT) ? MIN_PEER_PROTO_VERSION : MIN_PEER_PROTO_VERSION_AFTER_ZERONODE_PAYMENT_HF;
...
//        vRecv >> tx;
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CZeronode *pmn = mnodeman.Find(dstx.vin);
            if (pmn == NULL) {
                LogPrint("privatesend", "DSTX -- Can't find zeronode %s to verify %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if (!pmn->fAllowMixingTx) {
                LogPrint("privatesend", "DSTX -- Zeronode %s is sending too many transactions %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
            ...
            if (!dstx.CheckSignature(pmn->pubKeyZeronode)) {
