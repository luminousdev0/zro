...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if ((nHeight + 1 > 0) && (nHeight + 1 < 210000)) {
        ...
        } else if (nHeight >= Params().GetConsensus().nZeronodePaymentsStartBlock) {
            // Take some reward away from us
...
//            }
        ...
        // Update coinbase transaction with additional info about zeronode and governance payments,
        // get some info back to pass to getblocktemplate
        if (nHeight >= chainparams.GetConsensus().nZeronodePaymentsStartBlock) {
            CAmount zeronodePayment = GetZeronodePayment(nHeight, blockReward);
            coinbaseTx.vout[0].nValue -= zeronodePayment;
            FillBlockPayments(coinbaseTx, nHeight, zeronodePayment, pblock->txoutZeronode, pblock->voutSuperblock);
        }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL, false))
        return error("ZerobitcoinMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static ZerobitcoinMiner(const CChainParams &chainparams) {
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zerobitcoin-miner");
    ...
    try {
        ...
        if (!coinbaseScript || coinbaseScript->reserveScript.empty()) {
            LogPrintf("ZerobitcoinMiner stop here coinbaseScript=%s, coinbaseScript->reserveScript.empty()=%s\n", coinbaseScript, coinbaseScript->reserveScript.empty());
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in ZerobitcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running ZerobitcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        CheckWork(pblock, *pwallet, reservekey);
        ...
        LogPrintf("ZerobitcoinMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("ZerobitcoinMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void GenerateBitcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZerobitcoinMiner, boost::cref(chainparams)));
}
