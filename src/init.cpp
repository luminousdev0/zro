...
void Shutdown() {
    ...
    // TODO: https://github.com/zerobitcoinofficial/zerobitcoin/issues/182
    //    CFlatDB<CZeronodeMan> flatdb1("zncache.dat", "magicZeronodeCache");
    //    flatdb1.Dump(mnodeman);
    //    CFlatDB<CZeronodePayments> flatdb2("znpayments.dat", "magicZeronodePaymentsCache");
...
void InitLogging() {
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Zerobitcoin version %s\n", FormatFullVersion());
}
...
//    nCoinCacheUsage = nTotalCache; // the rest goes to in-memory cache
    ...
    fZNode = GetBoolArg("-zeronode", false);

    LogPrintf("fZNode = %s\n", fZNode);
    LogPrintf("zeronodeConfig.getCount(): %s\n", zeronodeConfig.getCount());

    if ((fZNode || zeronodeConfig.getCount() > 0) && !fTxIndex) {
    ...
    if (fZNode) {
        ...
        if (!GetArg("-zeronodeaddr", "").empty()) {
            // Hot Zeronode (either local or remote) should get its address in
            // CActiveZeronode::ManageState() automatically and no longer relies on Zeronodeaddr.
            return InitError(_("zeronodeaddr option is deprecated. Please use zeronode.conf to manage your remote zeronodes."));
        }

        std::string strZeronodePrivKey = GetArg("-zeronodeprivkey", "");
        if (!strZeronodePrivKey.empty()) {
            if (!darkSendSigner.GetKeysFromSecret(strZeronodePrivKey, activeZeronode.keyZeronode,
                                                  activeZeronode.pubKeyZeronode))
                return InitError(_("Invalid zeronodeprivkey. Please see documenation."));

            LogPrintf("  pubKeyZeronode: %s\n", CBitcoinAddress(activeZeronode.pubKeyZeronode.GetID()).ToString());
        } else {
            return InitError(
                    _("You must specify a zeronodeprivkey in the configuration. Please see documentation for help."));
        }
    }

    LogPrintf("Using Zeronode config file %s\n", GetZeronodeConfigFile().string());

    if (GetBoolArg("-znconflock", true) && pwalletMain && (zeronodeConfig.getCount() > 0)) {
    ...
    //lite mode disables all Zeronode and Darksend related functionality
    ...
    if (!flatdb1.Load(mnodeman)) {
        return InitError("Failed to load zeronode cache from zncache.dat");
    }
    ...
    if (mnodeman.size()) {
        ...
        if (!flatdb2.Load(mnpayments)) {
            return InitError("Failed to load zeronode payments cache from znpayments.dat");
        }
    } else {
        uiInterface.InitMessage(_("Zeronode cache is empty, skipping payments and governance cache..."));
    } */
