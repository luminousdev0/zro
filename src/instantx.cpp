...
// step 2) Top COutPointLock::SIGNATURES_TOTAL zeronodes per each spent outpoint push "txvote" message
...
//    if(!sporkManager.IsSporkActive(SPORK_2_INSTANTSEND_ENABLED)) return;
    ...
    // Ignore any InstantSend messages until zeronode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // Zeronodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetZeronodeRank(activeZeronode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown Zeronode %s\n", activeZeronode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Zeronode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasZeronodeVoted(itOutpointLock->first, activeZeronode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeZeronode.vin.prevout);
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  zeronode=%s new\n",
                    txHash.ToString(), vote.GetZeronodeOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  zeronode=%s seen\n",
                    txHash.ToString(), vote.GetZeronodeOutpoint().ToStringShort());
        }
        ...
        int nZeronodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapZeronodeOrphanVotes.count(vote.GetZeronodeOutpoint())) {
            mapZeronodeOrphanVotes[vote.GetZeronodeOutpoint()] = nZeronodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapZeronodeOrphanVotes[vote.GetZeronodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageZeronodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- zeronode is spamming orphan Transaction Lock Votes: txid=%s  zeronode=%s\n",
                        txHash.ToString(), vote.GetZeronodeOutpoint().ToStringShort());
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasZeronodeVoted(vote.GetOutpoint(), vote.GetZeronodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same zeronode.
                    // TODO: apply pose ban score to this zeronode?
                    ...
                    // pose ban score too.
                    LogPrintf("CInstantSend::ProcessTxLockVote -- zeronode sent conflicting votes! %s\n", vote.GetZeronodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
        // we have votes by other zeronodes only (so far), let's continue and see who will win
...
int64_t CInstantSend::GetAverageZeronodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapZeronodeOrphanVotes is empty
    if(mapZeronodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapZeronodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapZeronodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapZeronodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  zeronode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetZeronodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  zeronode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetZeronodeOutpoint().ToStringShort());
    ...
    // remove expired zeronode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itZeronodeOrphan = mapZeronodeOrphanVotes.begin();
    while(itZeronodeOrphan != mapZeronodeOrphanVotes.end()) {
        if(itZeronodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan zeronode vote: zeronode=%s\n",
                    itZeronodeOrphan->first.ToStringShort());
            mapZeronodeOrphanVotes.erase(itZeronodeOrphan++);
        } else {
            ++itZeronodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(CTxIn(outpointZeronode))) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown zeronode %s\n", outpointZeronode.ToStringShort());
        mnodeman.AskForMN(pnode, CTxIn(outpointZeronode));
        return false;
    }
    ...
    int n = mnodeman.GetZeronodeRank(CTxIn(outpointZeronode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated zeronode %s\n", outpointZeronode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Zeronode %s, rank=%d\n", outpointZeronode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Zeronode %s is not in the top %d (%d), vote hash=%s\n",
                outpointZeronode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    zeronode_info_t infoMn = mnodeman.GetZeronodeInfo(CTxIn(outpointZeronode));

    if(!infoMn.fInfoValid) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Zeronode: zeronode=%s\n", outpointZeronode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyZeronode, vchZeronodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchZeronodeSignature, activeZeronode.keyZeronode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeZeronode.pubKeyZeronode, vchZeronodeSignature, strMessage, strError)) {
...
{
    if(mapZeronodeVotes.count(vote.GetZeronodeOutpoint()))
        return false;
    mapZeronodeVotes.insert(std::make_pair(vote.GetZeronodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapZeronodeVotes.begin();
    while(itVote != mapZeronodeVotes.end()) {
...
bool COutPointLock::HasZeronodeVoted(const COutPoint& outpointZeronodeIn) const
{
    return mapZeronodeVotes.count(outpointZeronodeIn);
}
...
bool CTxLockCandidate::HasZeronodeVoted(const COutPoint& outpointIn, const COutPoint& outpointZeronodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasZeronodeVoted(outpointZeronodeIn);
}
