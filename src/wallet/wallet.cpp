...
//                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fZNode && pcoin->vout[i].nValue == ZERONODE_COIN_REQUIRED * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fZNode) found = pcoin->vout[i].nValue != ZERONODE_COIN_REQUIRED * COIN; // do not use Hot MN funds
                } else if (nCoinType == ONLY_1000) {
                    found = pcoin->vout[i].nValue == ZERONODE_COIN_REQUIRED * COIN;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (fZNode && out.tx->vout[out.i].nValue == ZERONODE_COIN_REQUIRED * COIN) continue; //zeronode input
...
bool CWallet::GetZeronodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
    ...
    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetZeronodeVinAndKeys -- Could not locate any valid zeronode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetZeronodeVinAndKeys -- Could not locate specified zeronode vin\n");
    return false;
}
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax,
    ...
    // ( bit on if present )
    // bit 0 - 100ZEROBITCOIN+1
    // bit 1 - 10ZEROBITCOIN+1
    // bit 2 - 1ZEROBITCOIN+1
    // bit 3 - .1ZEROBITCOIN+1
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        // zeronode-like input should not be selected by AvailableCoins now anyway
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if (fAnonymizable) {
                ...
                if (fZNode && wtx.vout[i].nValue == ZERONODE_COIN_REQUIRED * COIN) continue;
...
//                bool fFirst = true;
                ...
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl)) {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 ZRO.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 1000 ZRO.");
...
//                bool fAllowFree = false;					// No free TXs in ZRO
...
string CWallet::MintZerocoin(CScript pubCoin, int64_t nValue, CWalletTx &wtxNew, bool fAskFee) {
    ...
    if (fImporting || fReindex || !zeronodeSync.IsBlockchainSynced())
