...
/*
    At 15 signatures, 1/2 of the zeronode network can be owned by
...
private:
    ...
    //track zeronodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapZeronodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageZeronodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointZeronode;
    std::vector<unsigned char> vchZeronodeSignature;
    // local memory only
...
public:
    CTxLockVote() :
        ...
        outpoint(),
        outpointZeronode(),
        vchZeronodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}

    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointZeronodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointZeronode(outpointZeronodeIn),
        vchZeronodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointZeronode);
        READWRITE(vchZeronodeSignature);
    }
    ...
    COutPoint GetZeronodeOutpoint() const { return outpointZeronode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapZeronodeVotes; // zeronode outpoint - vote
...
public:
    ...
    bool HasZeronodeVoted(const COutPoint& outpointZeronodeIn) const;
    int CountVotes() const { return mapZeronodeVotes.size(); }
...
public:
    ...
    bool HasZeronodeVoted(const COutPoint& outpointIn, const COutPoint& outpointZeronodeIn);
