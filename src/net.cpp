...
CNode *ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, bool fConnectToZeronode) {
    if (pszDest == NULL) {
        // we clean zeronode connections in CZeronodeMan::ProcessZeronodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActiveZeronode::ManageState()
        if (IsLocal(addrConnect) && !fConnectToZeronode)
            return NULL;
        LOCK(cs_vNodes);
        // Look for an existing connection
        CNode *pnode = FindNode((CService) addrConnect);
        if (pnode) {
            // we have existing connection to this node but it was not a connection to zeronode,
            // change flag and add reference so that we can correctly clear it later
            if (fConnectToZeronode && !pnode->fZeronode) {
                pnode->fZeronode = true;
            }
            pnode->AddRef();
            return pnode;
        }
