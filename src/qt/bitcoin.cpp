...
{
    return QCoreApplication::translate("zerobitcoin-core", psz).toStdString();
}
...
#ifdef ENABLE_WALLET
    /// 7a. parse zeronode.conf
    std::string strErr;
    if(!zeronodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Zerobitcoin Core"),
                              QObject::tr("Error reading zeronode configuration file: %1").arg(strErr.c_str()));
        return EXIT_FAILURE;
    }
