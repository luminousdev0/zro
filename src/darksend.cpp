...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fZNode) {
            LogPrintf("DSACCEPT -- not a Zeronode!\n");
        ...
        CZeronode *pmn = mnodeman.Find(activeZeronode.vin);
        if (pmn == NULL) {
        ...
        CZeronode *pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        if (!dsq.CheckSignature(pmn->pubKeyZeronode)) {
        ...
        if (dsq.fReady) {
            if (!pSubmittedToZeronode) return;
            if ((CNetAddr) pSubmittedToZeronode->addr != (CNetAddr) pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Zeronode: pSubmittedToZeronode=%s, addr=%s\n", pSubmittedToZeronode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if (nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on zeronode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue
            q, vecDarksendQueue) {
                if (q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- Zeronode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if (pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Zeronode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from zeronode %s\n", dsq.ToString(), pmn->addr.ToString());
            if (pSubmittedToZeronode && pSubmittedToZeronode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if (!fZNode) {
            LogPrintf("DSVIN -- not a Zeronode!\n");
            PushStatus(pfrom, STATUS_REJECTED, ERR_NOT_A_MN);
            return;
        }
        ...
        if (fZNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a Zeronode!\n");
            return;
        }

        if (!pSubmittedToZeronode) return;
        if ((CNetAddr) pSubmittedToZeronode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Zeronode: pSubmittedToZeronode %s addr %s\n", pSubmittedToZeronode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fZNode) {
            LogPrintf("DSSIGNFINALTX -- not a Zeronode!\n");
            return;
        }
        ...
        if (fZNode) {
            // LogPrintf("DSFINALTX -- Can't run on a Zeronode!\n");
            return;
        }

        if (!pSubmittedToZeronode) return;
        if ((CNetAddr) pSubmittedToZeronode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Zeronode: pSubmittedToZeronode %s addr %s\n", pSubmittedToZeronode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fZNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a Zeronode!\n");
            return;
        }

        if (!pSubmittedToZeronode) return;
        if ((CNetAddr) pSubmittedToZeronode->addr != (CNetAddr) pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Zeronode: pSubmittedToZeronode=%s  addr=%s\n", pSubmittedToZeronode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus() {
    ...
    if ((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !zeronodeSync.IsBlockchainSynced())
    ...
    switch (nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to zeronode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if (nEntriesCount == 0) {
                ...
                if (nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to zeronode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if (nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if (nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if (nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to zeronode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
        case POOL_STATE_SIGNING:
...
//
// Check the mixing progress and send client updates if a Zeronode
//
...
void CDarksendPool::CommitFinalTransaction() {
    if (!fZNode) return; // check and relay final tx only on zeronode
    ...
    // create and sign zeronode dstx transaction
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeZeronode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to Zeronodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Zeronode
...
void CDarksendPool::CheckForCompleteQueue() {
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeZeronode.vin, GetTime(), true);
...
//
// Execute a mixing denomination via a Zeronode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fZNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a Zeronode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a Zeronode
    if (!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No Zeronode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from Zeronode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    strAutoDenomResult = _("Zeronode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if (nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Zeronode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
//
// After we receive the finalized transaction from the Zeronode, we must
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    ...
    // push all of our signatures to the Zeronode
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the zeronode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun) {
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No Zeronodes detected\n");
        strAutoDenomResult = _("No Zeronodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the Zeronode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecZeronodesUsed: size: %d, threshold: %d\n", (int) vecZeronodesUsed.size(), nThreshold_high);

    if ((int) vecZeronodesUsed.size() > nThreshold_high) {
        vecZeronodesUsed.erase(vecZeronodesUsed.begin(), vecZeronodesUsed.begin() + vecZeronodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecZeronodesUsed: new size: %d, threshold: %d\n", (int) vecZeronodesUsed.size(), nThreshold_high);
    }
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            CZeronode *pmn = mnodeman.Find(dsq.vin);
            if (pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq zeronode is not in zeronode list, zeronode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecZeronodesUsed.push_back(dsq.vin);
            ...
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to zeronode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to Zeronode and submit the queue request
            CNode *pnode = (pnodeFound && pnodeFound->fZeronode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
                pSubmittedToZeronode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to Zeronode.");
                continue;
            }
        }
    }
    ...
    while (nTries < 10) {
        CZeronode *pmn = mnodeman.FindRandomNotInVec(vecZeronodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if (pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random zeronode!\n");
            strAutoDenomResult = _("Can't find random Zeronode.");
            return false;
        }
        vecZeronodesUsed.push_back(pmn->vin);

        if (pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled / 5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this zeronode!"
                              " zeronode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Zeronode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fZeronode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
    ...
    strAutoDenomResult = _("No compatible Zeronode found.");
    return false;
}
...
bool CDarksendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    ...
    if (!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeZeronode.vin, GetTime(), false);
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID) {
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:
            return _("Not in the Zeronode list.");
        case ERR_MODE:
            return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:
            return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:
            return _("This is not a Zeronode.");
        case ERR_QUEUE_FULL:
            return _("Zeronode queue is full.");
        case ERR_RECENT:
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn &txin, const CPubKey &pubkey) {
    ...
    if (GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == ZERONODE_COIN_REQUIRED * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CDarksendQueue::Sign() {
    ...
    return CheckSignature(activeZeronode.pubKeyZeronode);
}

bool CDarksendQueue::CheckSignature(const CPubKey &pubKeyZeronode) {
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if (!darkSendSigner.VerifyMessage(pubKeyZeronode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad Zeronode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey &pubKeyZeronode) {
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry &entry) {
    ...
    CNode *pnode = FindNode(pSubmittedToZeronode->addr);
    if (pnode != NULL) {
...
void CDarksendPool::SetState(PoolState nStateNew) {
    if (fZNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a Zeronode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex) {
    ...
    if (!fLiteMode && zeronodeSync.IsZeronodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool() {
    ...
    while (true) {
        ...
        zeronodeSync.ProcessTick();

        if (zeronodeSync.IsBlockchainSynced() && !ShutdownRequested()) {

            nTick++;

            // make sure to check all zeronodes first
            mnodeman.Check();

            // check if we should activate or ping every few minutes,
            // slightly postpone first run to give net thread a chance to connect to some peers
            if (nTick % ZERONODE_MIN_MNP_SECONDS == 15)
                activeZeronode.ManageState();

            if (nTick % 60 == 0) {
                mnodeman.ProcessZeronodeConnections();
